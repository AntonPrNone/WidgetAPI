...
namespace API_Library
{
    ...
    public class WeatherAPILogic
    {
        ...
        public WeatherAPILogic(string apiKey = "bb422a0ecd67bfdb79b4808c42eafd12", string country = "Russia", string city = "Kazan", string units = "metric", int days = 5)
        {
            _apiKey = apiKey;
            _country = country;
            _city = city;
        ...
        public async Task<ThreeHoursWeather> GetWeeklyWeatherAsync()
        {
            using (var httpClient = new HttpClient())
            {
                ...
                {
                    var content = await response.Content.ReadAsStringAsync();
                    return JsonConvert.DeserializeObject<ThreeHoursWeather>(content);
                }
